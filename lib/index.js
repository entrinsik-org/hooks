...
internals.suffix = function (string, suffix) {
    return string + suffix.substring(0, 1).toUpperCase() + suffix.substring(1);
};
...
var createHook = function createHook() {
     ...
     * @param listener
     * @return {{remove: Function, pair: Function}} a registration object with: a remove function to remove the handler & a pair function to register related registration objects to remove their handler upon removing this handler and optionally make the pairing bidirectional
     */
    hook.add = function add(listener) {
        ...
        return {
            _name: listener.name,
            _paired: [],
            pair: function (related, bidirectional) {
                if (this._paired.indexOf(related) < 0) {
                    this._paired.push(related);
                    if (bidirectional) related.pair(this);
                }
                return this;
            },
            remove: function () {
                hook.remove.call(hook, listener);
                while (this._paired.length) {
                    this._paired.shift().remove();
                }
            }
        };
    };
...
Hooks.prototype.getHook = function(event) {
    if (!exports.hookTypes.hasOwnProperty(event)) throw new Error('Unknown event: ' + event);
...
/**
 * Composes a hook("beforeXYZ"), xyz(), hook("afterXYZ") sandwich. Any errors occurring within
 * the hookified function xyz() are emitted via hook("xyzError") and then re-thrown;
...
exports.hookify = function hookify(method, name) {
    ...
    var error = internals.suffix(name, 'error');
    ...
    return function () {
        ...
        var invokeMethod = function () {
            return P.resolve()
                ...
                .catch(function (err) {
                    // attempt to run 'xyzError' hook. gracefully ignore any further errors (ie. if error hook event isn't registered) & re-throw original
                    return P.try(function () {
                        // Unregistered Event error throws synchronously
                        return self.runHook.apply(self, [error].concat(err, args));
                    })
                        .catch(_.noop)
                        .finally(function () {
                            throw err;
...
exports.addEvents = function (events) {
    events = _.isArray(events) ? events : [].slice.call(arguments);
    events.forEach(function (event) {
        exports.hookTypes[event] = true;
    });
};
